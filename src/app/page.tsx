    ...
    <AppShell page="home">
      <div className={minimal ? "space-y-6" : "space-y-10"}>
        ...
        <section className={heroAlign}>
          ...
          {b.stadiumName && <p className="mt-2 text-sm opacity-50">🏟️ {b.stadiumName}</p>}
        ...
        {nextMatch && ui.showNextMatchCard && nextMatch.id !== data.club.info.liveMatchId && (
          ...
          >
            ...
            <p className="mt-2 opacity-70">
              {formatItDate(nextMatch.date, { weekday: "long", day: "numeric", month: "long" })}{" "}
              {nextMatch.time ? `· ${nextMatch.time}` : ""}
            </p>
            {nextMatch.location ? <p className="text-sm opacity-50">{nextMatch.location}</p> : null}
            <Link href={`/partita/${nextMatch.id}`} className="mt-3 inline-block text-sm font-semibold text-[var(--team-accent)]">
              Dettaglio
            </Link>
          </section>
        )}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            {bdays.length > 0 && (
              <div className="rounded-2xl border border-white/10 bg-[var(--team-card-bg)] p-4 team-card">
                ...
                <p className="mt-1 font-bold">
                  {bdays[0].player.name} tra {bdays[0].inDays}g
                </p>
              </div>
            )}
          </div>
        ...
        <section
          className={`grid gap-6 ${
            magazine ? "sm:grid-cols-2 lg:grid-cols-3" : cards.length > 3 ? "sm:grid-cols-2 lg:grid-cols-4" : "sm:grid-cols-3"
          }`}
        >
          {cards.map((card) => (
