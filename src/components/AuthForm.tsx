...
export default function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  async function loginAsAdmin(username: string, adminPass: string, adminPin: string) {
    if (!adminPin.trim()) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (adminLogin) {
        await loginAsAdmin(email.trim(), password, pin);
        return;
      }
    ...
    <div className="mx-auto max-w-md">
      <div className="rounded-3xl border border-white/10 bg-[var(--team-card-bg)] p-8 backdrop-blur-md">
        ...
        {googleEnabled ? (
          ...
          <button
            type="button"
            onClick={() => setError("Google non è ancora collegato. Usa email e password.")}
