    ...
    <div className="flex flex-wrap justify-center gap-x-2 gap-y-3">
      {players.map((p) => (
        <Link key={p.id} href={`/giocatore/${p.id}`} className="pressable">
          <PlayerToken player={p} captain={captainId === p.id} size="sm" siblings={players} />
        </Link>
      ))}
    </div>
  );
...
export default function CallupBoard() {
  ...
  const toggle = async (playerId: string) => {
    ...
    void hapticLight();
    try {
      await updateData({ club: { ...data.club, ...publishCallups(ids) } });
    } finally {
      setSaving(false);
    }
  };
  ...
  const clearAll = async () => {
    ...
    setSaving(true);
    try {
      await updateData({ club: { ...data.club, ...publishCallups([]) } });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-4">
            ...
            <div>
              ...
              {called.length > 0 && live && (
                <p className="mt-1 text-sm opacity-70">
                  {called.length} convocati · in Home ancora {daysLeft} {daysLeft === 1 ? "giorno" : "giorni"}
                </p>
              )}
              {canEdit && called.length > 0 && !live && (
                <p className="mt-1 text-sm text-amber-300">
                  Lista scaduta: pubblicala di nuovo per {CALLUP_VISIBLE_DAYS} giorni in Home.
                </p>
              )}
              {!canEdit && !live && (
                <p className="mt-1 text-sm opacity-60">Nessuna lista pubblicata in questo momento.</p>
              )}
            </div>
          </div>
              ...
              <>
                {!live && (
                  <button
                    type="button"
                    disabled={saving}
                    onClick={() => updateData({ club: { ...data.club, ...publishCallups(data.club.callupPlayerIds) } })}
        ...
        {data.club.callupMeeting && <p className="text-sm opacity-70">📍 {data.club.callupMeeting}</p>}
      ...
      {canEdit ? (
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm opacity-70">
              Tocca un giocatore per inserirlo nella lista. I convocati restano in Home per {CALLUP_VISIBLE_DAYS} giorni.
              {saving ? " Salvataggio…" : ""}
            </p>
          </div>
                    ...
                    return (
                      <button
                        ...
                        className={`pressable flex items-center gap-3 rounded-xl border px-4 py-3 text-left transition ${
                          on ? "border-[var(--team-accent)] bg-[var(--team-accent)]/15" : "border-white/10 bg-white/5 hover:border-white/30"
                        }`}
                      >
        ...
        <p className="text-sm opacity-50">Solo l'allenatore può selezionare i convocati dalla rosa.</p>
      )}
    </div>
  );
}
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-[var(--team-accent)]/30 bg-[var(--team-card-bg)] p-5 team-card">
      <div className="flex items-center justify-between gap-3">
        ...
        {live ? (
          <p className="text-xs opacity-60">in evidenza ancora {daysLeft} {daysLeft === 1 ? "giorno" : "giorni"}</p>
        ) : (
